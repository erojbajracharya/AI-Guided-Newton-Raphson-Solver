...
def main():
    ...
    with st.sidebar:
        ...
        ai_status = "✅ Connected" if ai.available else "⚠️ Not connected"
...
""")
    ...
    with tab1:
        ...
        with col_plot1:
            ...
            for i, tang in enumerate(result["plot_data"]["tangents"]):
                alpha = 0.3 + 0.7 * (
                    i / max(len(result["plot_data"]["tangents"]), 1)
                )
                ax1.plot(
                    tang["x"],
                    tang["y"],
        ...
        with col_plot2:
            ...
            ax2.set_title(
                "Convergence Rate (Error per Iteration)",
                fontsize=12,
                fontweight="bold"
            )
            ax2.legend()
    ...
    with tab2:
        ...
        with col_c:
            if len(result["history"]) > 1:
                st.metric(
                    "Last Step Size",
                    f"{result['history'][-1]['step_size']:.2e}"
                )
    ...
    with tab3:
        ...
        if not ai.available:
            st.warning(
                "AI Advisor is not connected. "
                "Add an API key in .env to enable AI guidance."
            )
        else:
...
### Convergence
- **Order:** Quadratic, so the error becomes much smaller near the root.
- **Condition:** f'(x) should not be zero near the root.
- **Initial Guess:** A good starting point helps the method converge faster.

### Advantages
- Very fast convergence
- Simple formula
- Works well when the initial guess is close to the root
...
|-------|-------|----------|
| Zero derivative | f'(xₙ) = 0 | Try a different starting point |
| Oscillation | Values bounce around | Try another initial guess |
